...
class Model_RNN:    
    ...
    def Build_RNN__(self, x, n_input , n_hidden1 , numOfClasses):
        """ 
        2 Stacked RNN Layers each with 32 nodes and one dense layer
        """
        print ('Building RNN network two layers each with 32 nodes and dense layer ... ' )        
            
        with tf.variable_scope('lstm') as scope:
            scope.reuse_variables()  
            
            w_hidden    = tf.Variable(tf.truncated_normal( [n_input    , n_hidden1]      , seed = Config_RNN.WeightSeed), name='W')
            b_hidden    = tf.Variable(tf.truncated_normal( [n_hidden1]     , seed = Config_RNN.WeightSeed) , name='B')
            ...
            n_layers = 2            
            lstm_last_output = self.create_lstm_multicell('lstm' , x , n_layers, n_hidden1)
            prediction       = self.RNN_DenseLayer(lstm_last_output , n_hidden1, numOfClasses)

        return prediction
    
    def create_lstm_multicell(self, name ,x ,  n_layers, nstates):
        def lstm_cell(i, s):
            print('creating cell %i in %s' % (i, s))
            return rnn_cell.LSTMCell(nstates)#, reuse=tf.get_variable_scope().reuse)
        
        lstm_multi_cell = rnn_cell.MultiRNNCell([lstm_cell(i, name) for i in range(n_layers)])
        outputs, states= rnn.rnn(lstm_multi_cell , x , dtype = tf.float32)             
            
        lstm_last_output= outputs[-1]  
        return lstm_multi_cell
    
    """ ******************************************************************************************************************************** """       
    def Build_RNN(self, x, n_input , n_hidden1 , numOfClasses):
        """ """
        lstm_last_output = self.RNN_HiddenLayers(x , n_input, n_hidden1)
        prediction       = self.RNN_DenseLayer(lstm_last_output , n_hidden1, numOfClasses)

        return prediction

    def RNN_HiddenLayers(self , x , n_input, n_hidden1, name='Hidden'):
        """ """
        with tf.name_scope(name):
            ...
            lstm_cell_2    = rnn_cell.LSTMCell(n_hidden1 , state_is_tuple = True , forget_bias = 1.0 )  
    ...
    """ ******************************************************************************************************************************** """               
    def Build_RNN_1Fold(self, x, n_input , n_hidden1 , numOfClasses):
        """ 
        2 Stacked RNN Layers each with 32 nodes and one dense layer
        """
        print ('Building RNN network two layers each with 32 nodes and dense layer ... ' )        
  
        w_hidden    = tf.Variable(tf.truncated_normal( [n_input    , n_hidden1]      , seed = Config_RNN.WeightSeed))
        b_hidden    = tf.Variable(tf.truncated_normal( [n_hidden1]     , seed = Config_RNN.WeightSeed))
        ...
        w_out      = tf.Variable(tf.truncated_normal ( [n_hidden1 , numOfClasses] , seed = Config_RNN.WeightSeed) )
        b_out      = tf.Variable(tf.truncated_normal( [numOfClasses]  , seed = Config_RNN.WeightSeed) )
    ...
    def Build_RNN_Salma(self, x, numOfClasses):
        """ 
        2 Stacked RNN layers (64, 32) and dropput 0.2 ni between: relu and sigmoid
        """
        print ('Building RNN network one layer with 64 nodes and dense layer ... ' )        
        ...
        weights   = {'hidden1': tf.Variable(tf.truncated_normal( [n_input    , n_hidden1]      , seed = Config_RNN.WeightSeed)),
                     'hidden2': tf.Variable(tf.truncated_normal( [n_hidden1  , n_hidden2]      , seed = Config_RNN.WeightSeed)),                    
                     'out'    : tf.Variable(tf.truncated_normal ( [n_hidden2 , numOfClasses] , seed = Config_RNN.WeightSeed) )}
        
        
        biases    = {'hidden1': tf.Variable(tf.truncated_normal( [n_hidden1]     , seed = Config_RNN.WeightSeed)),
                     'hidden2': tf.Variable(tf.truncated_normal( [n_hidden2]     , seed = Config_RNN.WeightSeed)),                    
                     'out'    : tf.Variable(tf.truncated_normal( [numOfClasses]  , seed = Config_RNN.WeightSeed) )}
    
        x = tf.transpose(x , [1 , 0 , 2] )
        x = tf.reshape  (x , [-1, Config_RNN.chunk_size] )
    
        x = tf.nn.relu(tf.matmul( x , weights['hidden1']) + biases['hidden1'])
        x = tf.nn.sigmoid(tf.matmul( x , weights['hidden2']) + biases['hidden2'])        
        x = tf.split    (0 , Config_RNN.n_chunks , x )
        
        lstm_cell_1    = rnn_cell.LSTMCell(n_hidden1 , state_is_tuple = True , forget_bias = 1.0)
        lstm_cell_1    = tf.nn.rnn_cell.DropoutWrapper(lstm_cell_1, input_keep_prob = tf.constant(0.2))  #allow drop out of cells to avoid overfitting
        lstm_cell_2    = rnn_cell.LSTMCell(n_hidden2 , state_is_tuple = True , forget_bias = 1.0)  
        lstm_cells     = rnn_cell.MultiRNNCell([lstm_cell_1, lstm_cell_2] , state_is_tuple = True)  
        outputs, states= rnn.rnn(lstm_cells , x , dtype = tf.float32)             
        
        lstm_last_output= outputs[-1]    
        prediction      =  tf.matmul ( lstm_last_output, weights['out']) + biases['out']
    
        return prediction
    
    def Build_RNN_Original(self, x, numOfClasses):
        
        print ('Building RNN network one layer with 64 nodes and dense layer ... ' )        
        layer = {'w':tf.Variable(tf.truncated_normal([Config_RNN.num_hidden_nodes1,numOfClasses] , seed = Config_RNN.WeightSeed)),
                 'b' :tf.Variable(tf.truncated_normal ( [numOfClasses]           , seed = Config_RNN.WeightSeed) )}
        ...
        """ LSTM cell """
        lstm_cell       = rnn_cell.LSTMCell (Config_RNN.num_hidden_nodes1 , state_is_tuple = True , activation = tf.nn.relu, forget_bias = 1.0, use_peepholes = True) #peepholes to allos LSTM to look backword 
        lstm_cell       = rnn_cell.MultiRNNCell([lstm_cell] * Config_RNN.num_hidden_layers, state_is_tuple=True)                                                #allow hidden layers for each cell
        
        input_dropout   = tf.constant(0.5)
        output_dropout  = tf.constant(0.2)
        ...
        outputs, states = rnn.rnn(lstm_cell,x, dtype = tf.float32)    #define static RNN that has input of equal length only﻿   
        
        #This is the dense layer                                        
        output          = tf.matmul(outputs[-1],layer['w']) + layer['b']
