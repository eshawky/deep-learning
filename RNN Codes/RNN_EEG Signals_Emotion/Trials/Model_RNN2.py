...
class Model_RNN:    
    ...
    def Build_RNN(self, x, n_input , n_hidden1 , numOfClasses):
        ...
        lstm_last_output = self.RNN_HiddenLayers(x              , n_input  , n_hidden1   , name='hidden')
        prediction       = self.RNN_DenseLayer(lstm_last_output , n_hidden1, numOfClasses, name='dense')

        return prediction

    def RNN_HiddenLayers(self , x , n_input, n_hidden1, name):
           
        with tf.name_scope(name):
            
            x              = self.RNN_HiddenLayer(x , n_input   , n_hidden1 , name='rnn1')
                       
            with tf.name_scope('multi'):
                
                n_layers        = Config_RNN.num_layers
                
                lstm_cell_list  = [self.get_lstm_cell(n_hidden1) for _ in range(n_layers)]
                lstm_cells      = rnn_cell.MultiRNNCell(lstm_cell_list, state_is_tuple = True )  
                ...
                #lstm_cell_2 = self.get_lstm_cell(n_hidden1)
    ...
    def RNN_HiddenLayer(self, x , n_input , n_hidden, name):
    ...
    def RNN_DenseLayer(self , lstm_last_output , n_hidden1, numOfClasses, name):
        """ """
        with tf.name_scope(name):
    ...
    def get_a_cell(self, n_hidden):
        """ """
        lstm_cell    = rnn_cell.LSTMCell(n_hidden , state_is_tuple = True , forget_bias = 1.0 )
        return lstm_cell
     
        with tf.name_scope('lstm'):      
            cell = tf.nn.rnn_cell.MultiRNNCell([get_a_cell(self.lstm_size) for _ in range(self.num_layers)])
