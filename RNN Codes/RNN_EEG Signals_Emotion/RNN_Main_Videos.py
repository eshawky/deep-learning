...
class RNN_Main(object):
    ...
    def main_process(self, userIndex):
        ...
        PreProcessData.main_process(userIndex) #to save with overlap
    ...
    def CrossValidation (self, user_path , label_path , userIndex , accuracy_file, n_input, n_hidden1,n_hidden2, x, NumOfClasses, hm_epochs):
        ...
        print ('AllData.shape  1' , AllData.shape)  
        print ('AllLabels.shape1' , AllLabels.shape)
        
        
        nVideos            = 12 #Config_EEG.NumOfVideos
        AllData, AllLabels = self.convertChunksIntoVideos(AllData, AllLabels, nVideos)
        print ('AllLabels.shape2' , AllLabels.shape)
        print ('AllData.shape  2' , AllData.shape)  
        ...
        for train_index, validate_index in kf.split(AllData):
            ...
            with tf.variable_scope('lstm_stacked') as scope:
                    ...
                    with tf.Session() as sess:
                        ...
                        AllData2, AllLabels2 = self.convertVideoIntoChunks(AllData, AllLabels)
                        print ('AllLabels.shape3' , AllLabels.shape)
                        print ('AllData.shape  3' , AllData.shape) 
                        
                        X_train, Y_train, X_validate, Y_validate = self.readRNNDataLabels(AllData2 , AllLabels2 , train_index, validate_index)
    ...
    def convertChunksIntoVideos(self , userChunks, userLabels, nVideos):
        
        videoChunks = []
        videoLabels = [] 
        for i in range (0, len(userChunks) , nVideos):
            """ """
            Chunks = userChunks[i : i + nVideos]
            videoChunks.append(Chunks)
            
            Labels = userLabels[i : i + nVideos]
            videoLabels.append(Labels)
            
        return np.array(videoChunks) , np.array(videoLabels) 
    
    def convertVideoIntoChunks(self , videoChunks, videoLabels):
        
        userChunks = []
        userLabels = [] 
        
        for video in videoChunks: #loop on videos
            for i in range (len(video)):
                chunk = video[i]
                userChunks.append(chunk)
        
        for video in videoLabels: #loop on videos
            for i in range (len(video)):
                label = video[i]
                userLabels.append(label)
                
        return np.array(userChunks), np.array(userLabels) 
        
    def readRNNDataLabels(self,train_test , labels , trainIndex, testIndex):
        """ """
        length      = len(train_test)
        trainIndex = self.updateTrainTestIndeces(train_index, chunk_size, length)
        testIndex  = self.updateTrainTestIndeces( testIndex, chunk_size, length)
    ...
    def updateTrainTestIndeces(self , train_index, chunk_size, length):

        train_index2 = []
        
        for i in range (len(train_index)):
            index = train_index[i]
            start = int( math.ceil(index * chunk_size) )
            end   = int( math.ceil(start + chunk_size) )
                        
            for j in range (start, end):
                if j < length :
                    train_index2.append(j)
