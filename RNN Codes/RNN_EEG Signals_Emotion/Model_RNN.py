...
class Model_RNN:    
    ...
    """ ******************************************************************************************************************************** """       
    def Build_RNN(self, x, n_input , n_hidden1 , n_hidden2, numOfClasses, n_chunks, chunk_size):

        x = tf.transpose(x , [1 , 0 , 2] )
        x = tf.reshape  (x , [-1, chunk_size] )
        
        lstm_last_output = self.RNN_HiddenLayers(x              , n_input  , n_hidden1   ,n_hidden2, n_chunks, name='hidden')
        prediction       = self.RNN_DenseLayer(lstm_last_output , n_hidden2, numOfClasses, name='dense')

        return prediction

    def RNN_HiddenLayers(self , x , n_input, n_hidden1, n_hidden2, n_chunks,name):
           
        with tf.name_scope(name):
            
            x              = self.RNN_HiddenLayer(x , n_input   , n_hidden1 , activation ='relu'   , name = 'rnn1')
            x              = self.RNN_HiddenLayer(x , n_hidden1 , n_hidden2 , activation ='sigmoid', name = 'rnn2')
                       
            with tf.name_scope('multi'):

                lstm_cell_1 = self.get_lstm_cell(n_hidden1)

                input_dropout   = tf.constant(Config_RNN.input_dropout)
                output_dropout  = tf.constant(Config_RNN.output_dropout)
                lstm_cell_1     = self.dropout_lstm_cell(lstm_cell_1 , input_dropout , output_dropout)
                
                lstm_cell_2 = self.get_lstm_cell(n_hidden2)
    ...
    def get_lstm_cell(self, n_hidden , name = 'lstm'):
        """ """
        with tf.name_scope(name):
            lstm_cell    = rnn_cell.LSTMCell(n_hidden , state_is_tuple = True , forget_bias = 1.0 )    
            #lstm_cell    = tf.nn.rnn_cell.DropoutWrapper(lstm_cell, input_keep_prob = tf.constant(0.2))
            
            return lstm_cell
    
    def dropout_lstm_cell (self, lstm_cell, input_dropout, output_dropout):
        
        #lstm_cell       = tf.nn.rnn_cell.DropoutWrapper(lstm_cell, input_keep_prob = input_dropout, output_keep_prob = output_dropout)  #allow drop out of cells to avoid overfitting
        #lstm_cell       = tf.nn.rnn_cell.DropoutWrapper(lstm_cell, output_keep_prob = output_dropout)  #allow drop out of cells to avoid overfitting
        lstm_cell       = tf.nn.rnn_cell.DropoutWrapper(lstm_cell, input_keep_prob = input_dropout)  #allow drop out of cells to avoid overfitting
        
        return lstm_cell
            
    def RNN_HiddenLayer(self, x , n_input , n_hidden, activation , name):
        
        with tf.name_scope(name):
            w_hidden    = tf.Variable(tf.truncated_normal( [n_input    , n_hidden]      , seed = Config_RNN.WeightSeed), name='W')
            b_hidden    = tf.Variable(tf.truncated_normal( [n_hidden]     , seed = Config_RNN.WeightSeed) , name='B')
            
            if activation =='relu':
                x = tf.nn.relu(tf.matmul( x , w_hidden) + b_hidden)
            
            if activation =='sigmoid':
                x = tf.nn.sigmoid(tf.matmul( x , w_hidden) + b_hidden)
            
            return x

    def RNN_DenseLayer(self , lstm_last_output , n_hidden, numOfClasses, name):
        """ """
        with tf.name_scope(name):
            w_out      = tf.Variable(tf.truncated_normal ( [n_hidden , numOfClasses] , seed = Config_RNN.WeightSeed) , name='W')
    ...
    """ ******************************************************************************************************************************** """       
    def train_NW(self, sess, optimizer, cost,X_train, Y_train, hm_epochs, n_chunks, chunk_size, x):
        ...
        for epoch in range(hm_epochs):
            ...
            for xx in range( numOfBatchs):                                                
                ...
                lastSamples = len(X_train) %  Config_RNN.batch_size
                    
                if xx < (numOfBatchs-1):
                    epoch_x    = np.array ( self.my_next_batch ( X_train  ,  Config_RNN.batch_size , index) )
                    epoch_y    = np.array ( self.my_next_batch ( Y_train  ,  Config_RNN.batch_size , index) ) 

                    epoch_x_new = []
                    for row in epoch_x:
                        """ """
                        row = row.tolist()
                        epoch_x_new.append(row)
                    
                    epoch_x_new    = np.array(epoch_x_new).reshape( (Config_RNN.batch_size , n_chunks, chunk_size ))                    

                if xx == (numOfBatchs-1) and lastSamples != 0:
                    ...
                    epoch_x_new    = np.array(epoch_x_new).reshape( (lastSamples  , n_chunks,chunk_size ))                                              
        
                _, c = sess.run ( [optimizer, cost], feed_dict = {x: epoch_x_new, Config_RNN.y: epoch_y}) 
    ...
    def test_NW_metrics(self,sess, accuracy, X_test, Y_test, prediction, n_chunks , chunk_size,x):
        ...
        for xx in range(numOfBatchs):
            lastSamples = len(X_test) %  Config_RNN.batch_size
                    
            if xx < (numOfBatchs-1):
                epoch_x    = np.array ( self.my_next_batch ( X_test  ,  Config_RNN.batch_size , index) )
                epoch_y    = np.array ( self.my_next_batch ( Y_test  ,  Config_RNN.batch_size , index) ) 
                epoch_x    = epoch_x.reshape( (Config_RNN.batch_size,   n_chunks , chunk_size ))                    
            
            elif xx == (numOfBatchs-1) and lastSamples != 0:
                epoch_x    = np.array ( X_test[-lastSamples:] )
                epoch_y    = np.array ( Y_test[-lastSamples:] )
                epoch_x    = epoch_x.reshape( (lastSamples , n_chunks , chunk_size ))            
                      
            #metrics
            y_p = tf.argmax(prediction, 1)
            Accuracy, y_pred = sess.run([accuracy, y_p], feed_dict={x:[i for i in epoch_x], Config_RNN.y:[i for i in epoch_y]})
            #print ("y_pred:", y_pred)
    ...
    def Train_Test_Random(self, x , y , X_train , Y_train , X_test , Y_test , prediction, n_chunks , chunk_size):
...
#         optimizer = tf.train.RMSPropOptimizer(0.001).minimize(cost)
        ...
        with tf.Session() as sess:
            ...
            print('Accuracy of test: ' , accuracy.eval({x : X_test.reshape((-1 , n_chunks , chunk_size )) , y : Y_test }) * 100)
    ...
    def update_splitIntoTrainTest(self, X_train, X_test, Y_train, Y_test):
        ...
        for i in range (len(X_train)):
            ...
            Y_train_new.append(self.process_label1(Y_train[i]) )
         
        for i in range (len(X_test)):    
            X_test_new.append(self.process_chunk(X_test[i]) )
            Y_test_new.append(self.process_label1(Y_test[i]) )
...
if __name__ == "__main__":
    ...
    userIndex  = 0
    Model_RNN.main_process(userIndex) 
